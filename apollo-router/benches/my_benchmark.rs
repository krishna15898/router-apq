...
async fn make_call(service: &apollo_router::services::SubgraphService) {
    ...
    let resp = service
        ...
        .oneshot(SubgraphRequest {
            ...
            subgraph_request: http::Request::builder()
                ...
                .body(Request::builder().query("query(
    $token: String
    $country: String
    $xHsPlatform: String
    ...
    $dynamicCollectionTitle: String
    $dynamicCollectionEngTitle: String
) {
    fetchContentCollection(
        contentCollectionRequest: {
            hsRequest: {
                token: $token
                countryCode: $country
                platformCode: $xHsPlatform
            }
            clientCapabilities: $clientCapabilities
            trayID: $trayID
            paginationToken: $paginationToken
            collectionType: $collectionType
            dynamicCollectionParam: {
                title: $dynamicCollectionTitle
                engTitle: $dynamicCollectionEngTitle
            }
        }
    ) {
        __typename
        ... on ContentCollectionResponseV2 {
            paginationToken
            trayId
            title {
                title {
